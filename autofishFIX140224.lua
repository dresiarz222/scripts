...
while task.wait() do
    ...
    repeat task.wait() until getBobber(castVector) or tick() - castTimeOut > 10
    local bobber = getBobber(castVector)
    if not bobber then continue end
    local bobberWaitTimeOut = tick()
    repeat task.wait() until bobber.CFrame.Y < 61.109 or tick() - bobberWaitTimeOut > 10
    local reelTimeOut = tick()
    repeat reel() task.wait() until plr.PlayerGui["_INSTANCES"].FishingGame.Enabled == true or tick()-reelTimeOut > 10
    local percentage = 0
