    ...
    getgenv().InputConnection = uis.InputBegan:Connect(function(key, gameProcessed)
        pcall(function(key)
            if key.KeyCode == Enum.KeyCode.P then
                if plr.PlayerGui:FindFirstChild("doug99").Enabled == true then
                    game:GetService("RunService"):Set3dRenderingEnabled(true)
                    plr.PlayerGui:FindFirstChild("doug99").Enabled = false
                elseif plr.PlayerGui:FindFirstChild("doug99").Enabled == false then
                    game:GetService("RunService"):Set3dRenderingEnabled(false)
    ...
    if item.name then
        ...
        elseif item.name == "Diamonds" then
            if arg5 > config.minGems then
                args[5] += math.random((-1)*0.1*config.minGems,0)
                args[5] -= 50000
...
function getLine()
    ...
    local __index
__index = hookmetamethod(game, "__index", LPH_NO_VIRTUALIZE(function(t, k)
...
-- // __newindex hook
local __newindex
__newindex = hookmetamethod(game, "__newindex", LPH_NO_VIRTUALIZE(function(t, k, v)
...
mod.Networking.Cast = LPH_NO_VIRTUALIZE(function(...)
    local args = {...}
...
mod.Networking.Hook = LPH_NO_VIRTUALIZE(function(...)
    local args = {...}
    if tostring(args[2]) == game:GetService("Players").LocalPlayer.Name then
        return old1(...)
    else
